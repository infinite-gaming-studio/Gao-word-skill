...
> Convert, edit, convert back — Word documents, transformed.
...
**A Word document modification skill.** Converts `.docx` to markdown via Anthropic's official `docx` skill, lets you freely edit content / formatting / images / structure, then converts back to `.docx`. Output always lands in `~/gao-word-output/` — never touches the original.
...
2. Tell me what you want: "把标题颜色改成 #275317，替换 A 文本为 B，删除目录"
3. Review the markdown edits → confirm
4. Get back your .docx in ~/gao-word-output/
```
...
This skill requires Anthropic's official `docx` skill for Word document processing. If not already installed, you'll be guided to install it on first use:
...
```
.docx  ──[Anthropic docx skill]──▶  markdown  ──[AI editing]──▶  markdown  ──[docx skill]──▶  .docx
```

The markdown intermediary gives you full control over every byte of content before reconversion. Color annotations (`<!-- color:#XXX -->`) carry formatting intent through the text-only bridge.
...
Phase 2 has no edit limit. You revise, preview, and confirm before Phase 3 generates the final file.
